...
class TheoryTagForm(ModelForm):
    ...
    class Meta:
        ...
        widgets = {'userId': forms.HiddenInput(),'subConcept':forms.HiddenInput()}
        labels = {
...
class QuesTagForm(Form):
    def __init__(self, *args, **kwargs):
        ...
        for i in range(1,self.n+1):
            self.fields['ques'+str(i)]=forms.CharField(widget=CKEditorUploadingWidget,required=False,label="Question"+str(i))
            self.fields['ques'+str(i)].disabled=True
            self.fields['ans'+str(i)]=forms.CharField(widget=CKEditorUploadingWidget,required=False,label="Answer"+str(i))
            self.fields['ans'+str(i)].disabled=True
...
class CrossConceptForm(Form):
    def __init__(self, *args, **kwargs):
        ...
        for i in range(1,self.n1+1):
            self.fields['q'+str(i)] = forms.CharField(widget=CKEditorUploadingWidget,required=False,label="Question"+str(i))
            self.fields['a'+str(i)] = forms.CharField(required=False,label="Answer"+str(i))
            self.fields['concept'+str(i)] = forms.CharField(widget=forms.HiddenInput(),required=False)
            self.fields['subconcept'+str(i)] = forms.CharField(widget=forms.HiddenInput(),required=False)
        for i in range(self.n1+1,self.n1+self.n2+1):
            self.fields['q'+str(i)] = forms.CharField(widget=CKEditorUploadingWidget,required=False,label="Question"+str(i))
            self.fields['a'+str(i)] = forms.IntegerField(required=False,label="Answer"+str(i))
            self.fields['concept'+str(i)] = forms.CharField(widget=forms.HiddenInput(),required=False)
            self.fields['subconcept'+str(i)] = forms.CharField(widget=forms.HiddenInput(),required=False)
